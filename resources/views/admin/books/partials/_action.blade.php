...
@endif
@if($libros->document['status_documents_id'] == 1 && Auth::user() != null && ((Auth::user()->getRoleNames() == 'Admin') || (Auth::user()->getRoleNames() == 'Librarian')) )
...
@if ($libros->document['status_documents_id'] != 2)
        @if(Auth::user() != null && (Auth::user()->getRoleNames() == 'Admin') || (Auth::user()->getRoleNames() == 'Librarian') )
                <a href="{{ $url_baja }}" class="btn-baja" value="rechazar" title="Rechazar: {{ $libros->document->title }}"><i class="fa fa-arrow-down text-danger"></i></a> |  
        @endif
@endif 
@if ($libros->document['status_documents_id'] != 1)
        @if(Auth::user() != null && (Auth::user()->getRoleNames() == 'Admin') || (Auth::user()->getRoleNames() == 'Librarian') )
                <a href="{{ $url_reactivar }}" class="btn-reactivar"  value="aceptar" title="Aprobar: {{ $libros->document->title }}"><i class="fa fa-arrow-up text-green"></i></a> |  
        @endif 
@endif
